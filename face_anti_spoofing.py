...
import datetime
import time
# time.sleep(15)  # tell me why I used delay here...
# parameters 
COUNTER, TOTAL = 0,0
...
for i_questions in range(0,limit_questions):
    ...
    index_question = random.randint(0,0)
    ...
    if counter_ok_questions ==  limit_questions:
        while True:
            im = show_image(cam,"Good Shubham Done",color = (0,255,0))
            cv2.imshow('liveness_detection',im)
            if cv2.waitKey(1) &0xFF == ord('q'):
                break
            # exit()
        ...
        while True:
            im = show_image(cam,"Sorry Shubham LIFENESS FAIL")
            cv2.imshow('liveness_detection',im)
            if cv2.waitKey(1) &0xFF == ord('q'):
                break
            exit()
